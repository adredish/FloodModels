classdef AgentM < handle
    
    properties (Constant)
        baselineNoiseAlpha = 0.02; % baseline noise for weight matrix - experimentally determined as memory module steady state
        baselineNoiseCost = 0.1; % baseline to put noise into the cost
        baselineAlpha = 0.1; % baseline storage for all experiences
    ...
    methods
        ...
        function self = AgentM(varargin)
            self.T = MemoryTimelineM(varargin{:});
            self.H = MemoryModule(self.T.nUnits);
            self.baselineStore();
        end
        ...
        function baselineStore(self)
            self.H.initializeMatrix('MemoryBaseline.mat');
        ...
        function RemindFlood(self, ~, floodTS, alpha, delta)  % alpha is change in salience, delta is change in cost
            ...
            self.wHA = self.wHA + delta * P_recalled.*rememberedCost/self.H.nU;
        end
...
%         end       
        ...
        function [rememory,recalledCost] = TestRecall(self, TS, floodTS)
            ...
            rememory = self.H.PatternSimilarity(P_recalled, P_then);
